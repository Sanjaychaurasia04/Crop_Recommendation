...
// ─── CONFIG ──────────────────────────────────────────────
const API_URL = import.meta.env.VITE_CROP_API_URL || 'https://your-app.onrender.com/predict';
...
const CROP_ICONS = {
  ...
  lentil:      '🫘', pomegranate: '🍎', banana:     '🍌',
  mango:       '🥭', grapes:      '🍇', watermelon: '🍉',
  muskmelon:   '🍈', apple:       '🍎', orange:     '🍊',
  papaya:      '🍈', coconut:     '🥥', cotton:     '🌿',
  jute:        '🌿', coffee:      '☕',
};
...
const CropRecommend = () => {
  ...
  const [result,  setResult]  = useState(null);   // { crop } | null
  ...
  const handlePredict = useCallback(async () => {
    ...
    try {
      ...
      setApiError(err.message.includes('fetch')
        ? 'Cannot reach the API. Check VITE_CROP_API_URL or CORS settings.'
        : err.message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="cr-container">
      ...
      <div className="cr-header">
        ...
        <p className="cr-subtitle">
          AI-powered crop advisor · 99.36% accuracy
        </p>
      </div>
      ...
      <div className="cr-grid">
        ...
        <Card className="cr-input-card">
          ...
          <div className="cr-section-label">
            <Icon name="droplet" size={13} color={theme.sage} />
            SOIL NUTRIENTS
          </div>
          ...
          {/* Climate */}
          <div className="cr-section-label">
            <Icon name="sun" size={13} color={theme.sage} />
            CLIMATE CONDITIONS
          </div>
          ...
          <div className="cr-actions">
            ...
            {(result || apiError || Object.keys(values).length > 0) && (
              <button className="cr-clear-btn" onClick={handleClear} aria-label="Clear">
        ...
        <div className="cr-results-section">
          {!result && !apiError && !loading && (
            <Card className="cr-empty-state">
              <div className="cr-empty-icon">
                <Icon name="seedling" size={48} color={theme.clay} />
              </div>
              <p className="cr-empty-text">
                Enter soil and climate values<br />to get your crop recommendation
              </p>
          ...
          {loading && (
            <Card className="cr-empty-state">
              <div className="cr-loading-ring" aria-label="Analyzing…" />
                  ...
                  {FIELDS.map(f => (
                    <div key={f.id} className="cr-summary-item">
                      ...
                      <span className="cr-summary-value">
                        {values[f.id]} <span className="cr-summary-unit">{f.unit}</span>
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <Card className="cr-tips-card">
                ...
                <div className="tips-content">
                  <div className="tip-item">
                    ...
                    <span>Verify recommendation with a local agronomist</span>
                  </div>
                  <div className="tip-item">
                    <Icon name="water" size={14} />
                    <span>Conduct a full soil test before sowing</span>
                  </div>
                  <div className="tip-item">
                    <Icon name="test"  size={14} />
                    <span>Check seasonal suitability for your region</span>
                  </div>
                </div>
              </Card>
            </>
...
const FieldInput = ({ field, value, hasError, onChange }) => (
  <div className="cr-field">
    <label className="cr-field-label" htmlFor={`cr-${field.id}`}>
      {field.label}
      <span className="cr-field-unit">{field.unit}</span>
    </label>
    <input
      id={`cr-${field.id}`}
      type="number"
    ...
    />
    {hasError && (
      <span className="cr-field-error" role="alert">Required</span>
    )}
  </div>
);
